			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="examDate" title="${message(code: 'school.examDate.label', default: 'Exam Date')}" />
					
						<th><g:message code="school.report.label" default="Report" /></th>
				...
				<g:each in="${schoolInstanceList}" status="i" var="schoolInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${schoolInstance.id}">${fieldValue(bean: schoolInstance, field: "examDate")}</g:link></td>
					
						<td>${fieldValue(bean: schoolInstance, field: "report")}</td>
